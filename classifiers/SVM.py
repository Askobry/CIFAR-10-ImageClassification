...
def svm_loss_vectorized(W, X, Y, reg):
    ...
    sample_label_value = patch_result[[range(patch_result.shape[0])], patch_Y]  # 1 X 200 切片操作，将得分array中标记位置的得分取出来作为新的array
    loss_array = np.maximum(0, patch_result - sample_label_value.T + delta)  # 200 X 10 计算误差
    loss_array[[range(patch_result.shape[0])], patch_Y] = 0  # 200 X 10 将label值所在的位置误差置零
